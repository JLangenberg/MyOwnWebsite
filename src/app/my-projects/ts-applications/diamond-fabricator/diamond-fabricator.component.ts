...
export class DiamondFabricatorComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    {
      let inputSize = Number(document.querySelector("input").value);
...
function generateDTop(diamondSize: number): string  {
  ...
  for (var row = 1; row < diamondSize; row++)   {
...
function generateDCenter(diamondSize: number): string   {
  var diamondCenter = "";
  for (var centerPieces = -1; centerPieces < (diamondSize*2); centerPieces++)    {
      diamondCenter += "-";
  }
  diamondCenter += "\n";
...
function generateDBottom(diamondSize: number): string   {
  var diamondBottom = "";
   ...
   for (var row = 0; row < diamondSize; row++)   {
...
function generateSpaces(amountOfSpaces: number): string {
  var spaces = "";
  for (var spacesGenerated = 0; spacesGenerated < amountOfSpaces; spacesGenerated++)  {
      spaces += " ";
  }
  return spaces;
}
